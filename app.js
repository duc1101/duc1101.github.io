...
myApp.controller('notesCtrl',function ($scope,$window,$sce) {
	...
	var GITHUB_OWNER = '';
	var GITHUB_REPO = '';
	var GITHUB_DEFAULT_BRANCH = null;
	...
	function getGithubFileMeta(token) {
		...
		if (GITHUB_DEFAULT_BRANCH) branch = GITHUB_DEFAULT_BRANCH;
		var url = 'https://api.github.com/repos/' + encodeURIComponent(GITHUB_OWNER) + '/' + encodeURIComponent(GITHUB_REPO) + '/contents/' + encodeURIComponent(NOTE_FILE_PATH) + '?ref=' + encodeURIComponent(branch);
	...
	function getGithubRepoInfo(token) {
		if (!GITHUB_OWNER || !GITHUB_REPO) return Promise.reject(new Error('Không thể xác định repo GitHub tự động.'));
		var url = 'https://api.github.com/repos/' + encodeURIComponent(GITHUB_OWNER) + '/' + encodeURIComponent(GITHUB_REPO);
		return fetch(url, { headers: getGithubApiHeaders(token) }).then(function(res) {
			if (!res.ok) throw new Error('Không thể lấy thông tin repo: ' + res.status);
			return res.json();
		});
	}
	...
	$scope.saveContent = async function() {
		...
		if (!token) return;
		try {
			var repoInfo = await getGithubRepoInfo(token);
			if (repoInfo && repoInfo.default_branch) {
				GITHUB_DEFAULT_BRANCH = repoInfo.default_branch;
			}
			var meta = await getGithubFileMeta(token);
			var body = {
				message: 'Update docs note',
				content: base64Encode($scope.noteContent || ''),
				branch: GITHUB_DEFAULT_BRANCH || GITHUB_BRANCH
			};
